...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Note> loadNotesFromDatabase() {
        ...
        try {
            ...
            long id, date;
            int iState, iPriority;
            String content;

            while (cursor.moveToNext()) {
                id = cursor.getLong(cursor.getColumnIndex(TodoNote._ID));
                date = cursor.getLong(cursor.getColumnIndex(TodoNote.COLUMN_DATE));
                iState = cursor.getInt(cursor.getColumnIndex(TodoNote.COLUMN_STATE));
                iPriority = cursor.getInt(cursor.getColumnIndex(TodoNote.COLUMN_PRIORITY));
                content = cursor.getString(cursor.getColumnIndex(TodoNote.COLUMN_CONTENT));

                Note note = new Note(id);
                note.setDate(new Date(date));
                note.setState(State.from(iState));
                note.setPriority(Priority.from(iPriority));
